...
if SuccessFlag<1 %Apply penalty if path is invalid
    fitness = 10E32;
else
    %Get interpolated path
    [X_seq,Y_seq,Z_seq,x_seq,y_seq,z_seq] = GetThePathLine(position,NodesNumber,startPoint,endPoint); 
    %% Calculate path length (fitness) of discrete points from cubic spline
    dx = diff(X_seq);
    dy = diff(Y_seq);
    dz = diff(Z_seq);

    PathLength = sum(sqrt(dx.^2 + dy.^2 + dz.^2));%Path length

    Height = sum(((Z_seq - mean(Z_seq)).^2).^0.5); %Height evaluation
    ...
    phi = pi/2;
    Curve = sum(cos(phi)-C);  %Turning angle evaluation
    %Set weights
    w1 = 0.4;
    w2 = 0.4;
    w3 = 0.2;
